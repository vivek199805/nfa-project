...
export const updateEditorService = async ({ payload, userId }) => {
  const editor = await Editor.findOne({ _id: payload.id, client_id: userId });
...
export const listEditorService = async ({ payload, userId }) => {
  ...
  const allEditor = await Editor.find(whereTo);
  if (!allEditor || allEditor.length === 0) {
    return { message: "No result found.!!", statusCode: 203 };
  }

  return { message: "Success", statusCode: 200, data: allEditor };
};

export const getEditorService = async ({ id, userId }) => {
  const editor = await Editor.findOne({ _id: id, client_id: userId });
