...
const updateEntryById = async (req, res) => {
  try {
    ...
    let stepHandler = {
        ...
        [Common.stepsBestFilmCritic().ARTICLE]: async (existingEntry, payload) => await handlePublisherStep(existingEntry, payload),
 ...
 const finalSubmit = async (req, res) => {
  ...
  const handleBestFilmCriticStep = async (data, payload) => {
    ...
    if (lastId) {
      ...
      if (!checkForm.active_step ||checkForm.active_step < Common.stepsBestFilmCritic().CRITIC_DETAILS) {
 ...
 const  handleCriticStep = async (data, payload) => {
    ...
    if (lastId) {
      ...
      if (
        ...
        checkForm.active_step < Common.stepsBestFilmCritic().CRITIC
      ) {
 ...
 const handlePublisherStep = async (data, payload) => {
    ...
    if (lastId) {
      ...
      if (
        !checkForm.active_step ||
        checkForm.active_step < Common.stepsBestFilmCritic().PUBLISHER
      ) {
 ...
 const handleDeclarationStep = async (data, payload) => {
    ...
    if (
      !checkForm.active_step ||
      checkForm.active_step < Common.stepsBestFilmCritic().DECLARATION
    ) {
 ...
 const  bestFilmCriticById = async (req, res) => {
    const payload = {
      ...req.params,
      user: req.user,
    };
